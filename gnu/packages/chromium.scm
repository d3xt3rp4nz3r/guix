...
(define (debian-patch name path revision hash)
  "Return an <origin> from Debians Chromium repository at REVISION,
residing at PATH with the given HASH.  NAME will be used as file name."
  (origin
    (method url-fetch)
    (uri (string-append
          "https://anonscm.debian.org/cgit/pkg-chromium/pkg-chromium.git"
          "/plain/debian/patches/" path "?id=" revision))
    (sha256 (base32 hash))
    (file-name name)))

(define (gentoo-patch name path revision hash)
  "Return an <origin> from Gentoos Chromium repository at REVISION,
residing at PATH with the given HASH.  NAME will be used as file name."
  (origin
    (method url-fetch)
    (uri (string-append
          "https://gitweb.gentoo.org/repo/gentoo.git/plain/www-client"
          "/chromium/files/" path "?id=" revision))
    (sha256 (base32 hash))
    (file-name name)))
...
;; package the latest master branch as of 2017-11-03.
  ...
  (package
    ...
    (source (origin
              ...
              (uri (git-reference
                    ...
                    (commit "eb7d431cb55c8254fcd0a362e269c1d7a25409ba")))
              (file-name "libvpx-for-chromium-checkout")
              (sha256
               (base32
                "18ybwp17dzdb2li3ga9arxzxddm7d891p4yz04wywik559lfm2n6"))))
...
(define %chromium-gn-bootstrap.patch
  (gentoo-patch "chromium-gn-bootstrap.patch"
...
(define %chromium-gcc-5-compat.patch
  (gentoo-patch "chromium-gcc-5-compat.patch"
                "chromium-gcc5-r3.patch"
                "5c9cf110bd61fa287a5c536760b5d8ed13f65d52"
                "0qwl396w2bnc4ww71q3621chh9rfnw1m3w6nbd55sbhq8yz6jnx0"))

(define %chromium-system-nspr.patch
  (debian-patch "chromium-system-nspr.patch"
                "system/nspr.patch"
                "b8ccccd87e48ec462048f18446448652bd9016da"
                "1czjqjx3nsgj44y0kv8686qasjvfff7chn64i7ibr3hx7vx0jf9g"))

(define %chromium-system-libevent.patch
  (debian-patch "chromium-system-libevent.patch"
                "system/event.patch"
                "b8ccccd87e48ec462048f18446448652bd9016da"
                "0f9r7m84fzf5x4a1k26jf3mgkba8285lg8ypnn39vhwhw2ik3q0r"))

(define %chromium-external-components.patch
  (debian-patch "chromium-dont-load-external-components.patch"
                "disable/external-components.patch"
                "b8ccccd87e48ec462048f18446448652bd9016da"
                "0ay47k97q25r7mqpp6vfkv108n0zvsdl8n1l6y1nbhmnkpp62j1v"))
...
(define-public chromium
  (package
    ...
    (source (origin
              ...
              (patches (append (list %chromium-external-components.patch
